Shader "Universal Render Pipeline/ShopperRectDecalProjection"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float _Curvature; float4 _BaseColor; float4 _SurfaceColor; float _AlphaClip;
            ...
            Varyings vert(Attributes IN){ Varyings OUT; OUT.positionHCS=TransformObjectToHClip(IN.positionOS.xyz); OUT.positionOS=IN.positionOS.xyz; OUT.normalOS=IN.normalOS; return OUT; }
            float4 frag(Varyings IN):SV_Target{
                float3 pos=IN.positionOS; float3 normalOS=normalize(IN.normalOS); float3 center=_PlaneCenterOS.xyz;
                ...
                float depthAllowance=max(_Curvature*0.05,0.01); if(abs(nLocal-_PlaneOffset)>depthAllowance) return _SurfaceColor;
