...
public class DecalManager : MonoBehaviour
{
    ...
    private ModelProjectionZone _projectionZone;
    ...
    private void Awake()
    {
        UrpDecalAutoCheck.ValidateOnce();

        _mainCamera = Camera.main;
        ResolveProjectionZoneFromTargetModel();
        RebuildTransformService();

        if (_targetModel == null)
        {
            Debug.LogWarning("DecalManager: target model is not set. Assign it via SetTargetModel() from ModelManager.");
        }
    ...
    private void ResolveProjectionZoneFromTargetModel()
    {
        _projectionZone = _targetModel != null
            ? _targetModel.GetComponentInChildren<ModelProjectionZone>(true)
            : null;
    }

    private void RebuildTransformService()
    {
        if (_mainCamera == null)
            return;

        _transformService = _projectionZone != null
            ? new DecalTransformService(_mainCamera, _modelLayer, _projectionZone, _uiToWorldScale)
            : new DecalTransformService(_mainCamera, _modelLayer, _uiToWorldScale);
    }
